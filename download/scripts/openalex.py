...
from utils import sp, log, safe_filename, ensure_output_dir, validate_pdf, FailedRecord
...
def search_works(params):
    ...
    if params["field"]:
        filters.append(f"concept.display_name:{params['field']}")
    ...
    try:
        ...
        for r in results:
            ...
            pdf_url = ""
            if oa_location.get("is_oa") and oa_location.get("pdf_url"):
...
def main(args_text: str):
    ...
    for i, p in enumerate(papers, 1):
        if p["is_oa"] and p["pdf_url"]:
            result = download_pdf(p["pdf_url"], p["title"], i, output_dir)
            if result:
                downloaded += 1
            else:
                failed.add(title=p["title"], doi=p["doi"], link=p["pdf_url"], source="OpenAlex", reason="Download failed")
        elif not p["is_oa"]:
