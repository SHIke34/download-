...
from utils import sp, log, safe_filename, ensure_output_dir, save_json, connect_playwright_async
...
def parse_args(args_text: str) -> dict:
    ...
    sortBy: citations (默认) / date / relevance
    """
    params = {
        "query": DEFAULT_QUERY,
    ...
    if len(parts) >= 3 and parts[2]:
        sort_map = {
            "被引": "citations", "citations": "citations",
            "日期": "date", "date": "date",
            "相关度": "relevance", "relevance": "relevance",
        }
        params["sort"] = sort_map.get(parts[2].lower(), DEFAULT_SORT)
...
async def try_download_paper(page, doc_id, title, output_dir, idx):
    """尝试三种方式下载单篇论文"""
    ...
    if "access denied" in page_text or ("your institution" not in page_text and "sign in" in page_text and "access" in page_text):
        log("IEEE-VPN", "      [需要机构授权] 请在浏览器中通过 VPN 或机构登录后重试")
        return False
...
async def main_async(args_text: str):
    ...
    log("IEEE-VPN", "=" * 50)
    log("IEEE-VPN", f"Query: {params['query']}")
    log("IEEE-VPN", f"Year: {params['start_year']}-{params['end_year']}")
    log("IEEE-VPN", f"Sort: {params['sort']} | Count: {params['count']} | Output: {output_dir}")
    ...
    try:
        ...
        papers = await search_ieee(page, params["query"], params["start_year"], params["end_year"], params["sort"])
        ...
        for idx, paper in enumerate(papers[: params["count"]], 1):
            ...
            if await try_download_paper(page, doc_id, title, output_dir, idx):
                success += 1
